...
class Series(object):
    ...
    def series(self, file, fundDict, sheetname):
        ...
        for i in range(1, flows.nrows):
            ...
            if len(code) > 0:
                ...
                if fundInfo == 0:
                    ...
                    # 没信息就是股票
                    stock = flows.cell(i, flows.ncols - 1).value
                    if not isinstance(stock, unicode):
                        allStock += stock
                        allMoney += stock
                    continue
                money = flows.cell(i, flows.ncols - 1).value
        ...
        print '{}表中账户共有{}, 股票占比{:.2f}, 债券占比{:.2f}, 现金占比{:.2f}, 商品占比{:.2f}'.format(sheetname, allMoney, allStock / allMoney,
                                                                              allBond / allMoney,
                                                                              allCash / allMoney,
                                                                              allCommodity / allMoney)
...
if __name__ == "__main__":
    ...
    if file is not None:
        ...
        t.series(file, fundDict, '家庭账户时序表')
        t.series(file, fundDict, '个人账户时序表')